...
def build_keyword_library_df(result_df: pd.DataFrame, metadata: dict[str, Any] | None = None) -> pd.DataFrame:
    ...
    for _, row in result_df.iterrows():
        rows.append(
            {
                "原搜索词": safe_cell(row.get("原搜索词", "")),
                "关键词": safe_cell(row.get("原搜索词", "")),
                "中文翻译": safe_cell(row.get("中文翻译", "")),
                "词意图类型": safe_cell(row.get("词意图类型", "")),
                "产品线": product_line,
                ...
                "需求评分": safe_cell(row.get("需求评分", "")),
                "点击转化效率评分": safe_cell(row.get("点击转化效率评分", "")),
                "产品相关性评分": safe_cell(row.get("产品相关性评分", "")),
                "风险评分": safe_cell(row.get("风险评分", "")),
                "综合优先级评分": safe_cell(row.get("综合优先级评分", "")),
                "分类结果": safe_cell(row.get("分类结果", "")),
                "推广优先级": safe_cell(row.get("推广优先级", "")),
                "建议动作": safe_cell(row.get("建议动作", "")),
                "是否进入关键词库": safe_cell(row.get("是否进入关键词库", "")),
                "词库类型": safe_cell(row.get("词库类型", "")),
                "为什么这样分类": safe_cell(row.get("为什么这样分类", "")),
                "命中的规则": safe_cell(row.get("命中的规则", "")),
